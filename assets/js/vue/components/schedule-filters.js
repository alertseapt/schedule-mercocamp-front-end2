...
const ScheduleFilters = {
    ...
    computed: {
        hasActiveFilters() {
            return Object.values(this.localFilters).some(value => value && value.toString().trim() !== '');
        },
        
        filterCount() {
            return Object.values(this.localFilters).filter(value => value && value.toString().trim() !== '').length;
        }
    },
    ...
    methods: {
        ...
        handleInputChange() {
            // Aplicar filtros automaticamente após delay
            ...
            <div v-show="expanded" class="filter-content">
                ...
                <div class="quick-filters">
                    ...
                    <button 
                        ...
                        @click="localFilters.status = 'Solicitado'; handleSelectChange()">
                        Solicitados
                    </button>
                    <button 
                        class="btn btn-sm btn-outline-primary"
                        @click="localFilters.status = 'Agendado'; handleSelectChange()">
                        Agendados
                    </button>
                    <button 
                        class="btn btn-sm btn-outline-success"
                        @click="localFilters.status = 'Recebido'; handleSelectChange()">
                        Recebidos
                    </button>
                    <button 
                        class="btn btn-sm btn-outline-warning"
                        @click="localFilters.date_from = new Date().toISOString().split('T')[0]; handleSelectChange()">
                        Hoje
                    </button>
                    <button 
                        class="btn btn-sm btn-outline-secondary"
                        @click="
                            ...
                            localFilters.date_from = lastWeek.toISOString().split('T')[0];
                            handleSelectChange();
                        ">
                        Última semana
                    </button>
